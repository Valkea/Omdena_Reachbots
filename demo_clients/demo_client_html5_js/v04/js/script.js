...
function postPictures(files, action, callback) {
	...
	/*if(action == "predict_defects"){

		extra_v = document.getElementById('ZZ').value;
		console.log("ZZ:", extra_v);
		if (trade_v != ''){
			formData.append('ZZ', extra_v);
		}
	}*/
...
function showResult( files, jsons ){
	...
	jsons_cell.innerHTML = "<br><strong>Defects</strong><br>"+JSON.stringify(jsons['defects_json'])
}
