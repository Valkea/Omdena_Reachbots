...
// const api_url = "http://ec2-34-244-123-190.eu-west-1.compute.amazonaws.com:5000/";
...
function shoot(){
	...
	var canvas = capture(video, 0.25);
            canvas.onclick = function(){
		    window.open(this.toDataURL());
        };
	output.innerHTML = '';
	output.appendChild(canvas);
	//x = canvas.getContext('2d').getImageData(0, 0, 50, 100).data;
	var x = canvas.toDataURL("image/jpeg", 1.0).replace("data:image/jpeg;base64,","");
	var x = canvas.toDataURL("image/jpeg", 1.0);
	/*const base64Image = FileSystem.readAsStringAsync(canvas, {
    		encoding: FileSystem.EncodingType.Base64,
  	})
  	x = `data:image/png;base64,${base64Image}`
	*/
	/*x = undefined
	canvas.toBlob((blob) => {
	...
	console.log(x)
  	const newImg = document.createElement("img");
	newImg.src = x; // URL.createObjectURL(x);
